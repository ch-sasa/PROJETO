...
def listar_mp4s(pasta_entrada: Path) -> list[Path]:
    if not pasta_entrada.exists():
        pasta_entrada.mkdir(parents=True, exist_ok=True)

    return sorted(
        arquivo.resolve()
        for arquivo in pasta_entrada.glob("*.mp4")
        if arquivo.is_file()
    )
...
def preparar_letra_compativel(
    ...
    letra_original = pasta_letras / f"{arquivo_mp4.stem}.txt"
...
def converter_mp4_para_mp3(
    ...
    nome_compativel = normalizar_nome_arquivo(arquivo_mp4.stem)
    ...
    if arquivo_mp3.exists() and not sobrescrever:
        ...
        preparar_letra_compativel(arquivo_mp4, nome_compativel, pasta_letras)
        return True
    ...
    print(f"[CONVERTENDO] {arquivo_mp4}")
    ...
    if resultado.returncode != 0:
        print(f"[ERRO] Falha ao converter: {arquivo_mp4}")
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Converte arquivos .mp4 para .mp3 usando ffmpeg."
    )
    parser.add_argument(
        "--entrada",
        default="videos-para-converter",
        help="Pasta unica de onde os MP4 serao lidos. Padrao: videos-para-converter",
    )
    parser.add_argument(
        "--saida",
        default="musicas",
        help="Pasta onde os MP3 serao salvos. Padrao: musicas",
    )
    parser.add_argument(
        "--letras",
        default="letras",
        help="Pasta das letras usadas pelo sincronizar-todas.bat. Padrao: letras",
    )
    parser.add_argument(
        "--bitrate",
        default="192k",
    ...
    arquivos_mp4 = listar_mp4s(pasta_entrada)

    if not arquivos_mp4:
        print(f"[AVISO] Nenhum arquivo .mp4 encontrado em: {pasta_entrada}")
        return 1
    ...
    for arquivo_mp4 in arquivos_mp4:
        if converter_mp4_para_mp3(
            ffmpeg,
            arquivo_mp4,
            pasta_saida,
            pasta_letras,
